...
export function ScrollCard({ card, action }: CardProps) {
	...
	return (
		<motion.div
			ref={ref}
			className="sm:h-[450px] md:h-[550px] lg:h-[500px] xl:h-[550px] relative rounded-3xl cursor-pointer group p-2 bg-neutral-800 border-1 border-neutral-700"
		...
		>
			<a href={card.link} target="_blank" rel="noreferrer">
				...
				<div
					className="w-full h-full px-8 sm:px-16 flex flex-col justify-between overflow-hidden rounded-2xl"
					style={{
						backgroundColor: "white",
						backgroundImage: "linear-gradient(190deg, " + card.primaryColor + " 20%, " + card.secondaryColor + " 60%, " + card.secondaryColor + "40 100%)"
					}}
				>
					<p className="hidden sm:block text-2xl mt-12 mb-8" style={{ color: card.textColor }}>{card.text}</p>
